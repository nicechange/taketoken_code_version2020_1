...
{% block table %}
<table{% if table.attrs %} {{ table.attrs.as_html }}{% endif %} cellspacing="1" id="ordt" class="tablesorter">
    {% block table.thead %}
    ...
    <thead>
        ...
        {% for column in table.columns %}
            {% if column.orderable %}
            <th {{ column.attrs.th.as_html }}><a>{{ column.header }}</a></th>
            {% else %}
        ...
        <tr {{ row.attrs.as_html }}>
            {% for column, cell in row.items %}
                <td {{ column.attrs.td.as_html }}>{% if column.localize == None %}{{ cell }}{% else %}{% if column.localize %}{{ cell|localize }}{% else %}{{ cell|unlocalize }}{% endif %}{% endif %}
                  {% if row.items.owner.is_authenticated %}
                                                <span class="m-l label label-success">Online</span>
                                                {% endif %}
                </td>
            {% endfor %}
        </tr>
