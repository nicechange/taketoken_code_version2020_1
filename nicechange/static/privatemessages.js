...
function activate_chat(thread_id, user_name, number_of_messages) {
    $("div.chat form.message_form textarea").focus();
    ...
    function start_chat_ws() {
        ws = new WebSocket('ws://' + window.location.host + ':8888' + '/' + thread_id + "/");
        ws.onmessage = function(event) {
            ...
            var time = $.map([date.getHours(), date.getMinutes(), date.getSeconds()], function(val, i) {
             ...
             if (message_data.text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g, '<br />') == "Средства отправлены")
            {
                $("div.chat div.conversation").append('<p class="deal__chat__item sended">' +message_data.text.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g, '<br />')+'</p>');
            }else {
                 if (message_data.sender == user_name) {
                     $("div.chat div.conversation").append('<p class="deal__chat__item inside">' + message_data.text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br />') + '</p>');
                 }
                 else {
                     $("div.chat div.conversation").append('<p class="deal__chat__item outside">' + message_data.text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/\n/g, '<br />') + '</p>');
                 }
             }
    ...
    function send_message() {
        var textarea = $("input#message_textarea");
        if (textarea.val() == "") {
            return false;
        }
  ...
  return false;
    ...
    $("input#message_textarea").keydown(function (e) {
        // Ctrl + Enter
        if ( e.keyCode == 13) {
            send_message();
        }
